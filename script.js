...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inirMarqueeAnimation(initialSlide.querySelector(".marquee-container h1"));

    function updateProgressBars(progress) {
        const progressBars = document.querySelectorAll(".progress-bar");
        progressBars.forEach((bar, index) => {
            const barProgress = Math.min(Math.max(progress * 5 - index, 0), 1);
            bar.computedStyleMap.setProperty("--progress", barProgress);
        });
    }
    ...
    function createAndAnimateSlide(index, isScrollingForward) {
        ...
        newSlide.innerHTML = `
            <div class="slide-img">
                <img src="slide_2-${index + 1}.png" alt="" />
            </div>
        ...
        gsap.killTweensOf(currentSldie);
        ...
        onUpdate: (self) => {
            ...
            if (targetSlideIndex === activeSlideIndex) {
                ...
                try {
                    createAndAnimateSlide(targetSldieIndex, isScrollingForward);
